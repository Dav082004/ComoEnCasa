...
const AdminReportes = () => {
  ...
  const handleCategoryChange = useCallback(
    (categoryName) => {
      ...
      if (categoryName === "all") {
        ...
        const productosDeEstaCategoria =
          filteredData.gananciasPorProducto.filter((producto) => {
            ...
            if (productoOriginal && productoOriginal.categoriaId) {
              const nombreCategoria =
                categoriaMap[productoOriginal.categoriaId];
    ...
    [filteredData.gananciasPorProducto, productos, categoriaMap]
  );
    ...
    pedidosFiltrados.forEach((pedido) => {
      ...
      if (pedido.detalles && pedido.detalles.length > 0) {
        pedido.detalles.forEach((detalle) => {
          ...
          const montoVenta =
            detalle.subtotal || detalle.precioUnitario * cantidadVendida;
          ...
          if (producto && producto.categoriaId) {
            ...
            nombreCategoria =
              categoriaMap[producto.categoriaId] || "Sin categoría";
          }
          ...
          gananciasPorProducto[nombreProducto].costo +=
            costoUnitario * cantidadVendida;
          gananciasPorProducto[nombreProducto].venta += montoVenta;
          gananciasPorProducto[nombreProducto].margen =
            gananciasPorProducto[nombreProducto].venta > 0
              ? (gananciasPorProducto[nombreProducto].ganancia /
                  gananciasPorProducto[nombreProducto].venta) *
                100
              : 0;
        });
      }
  ...
  useEffect(() => {
    ...
    if (productos.length > 0) {
      ...
      const categoriasEnProductos = [
        ...new Set(productos.map((p) => p.categoriaId)),
      ];
      console.log(
        "🏷️ Categorías encontradas en productos:",
        categoriasEnProductos
      );
      ...
      categoriasEnProductos.forEach((catId) => {
        ...
        console.log(
          `📂 Categoría ID ${catId}: ${
            nombreCategoria || "NOMBRE NO ENCONTRADO"
          } - ${productosEnCategoria.length} productos`
        );
        if (productosEnCategoria.length > 0) {
          console.log(
            `   Productos en categoría ${catId}:`,
            productosEnCategoria.map((p) => p.nombre)
          );
        }
      });
    }
  ...
  useEffect(() => {
    ...
    filteredData.gananciasPorProducto,
    selectedCategory,
    productos,
    categoriaMap,
  ...
  const setQuickFilter = (type) => {
    ...
    switch (type) {
      ...
      case "last3Months":
        ...
        const from3 = `${threeMonthsAgo.getFullYear()}-${(
          threeMonthsAgo.getMonth() + 1
        )
          .toString()
          .padStart(2, "0")}`;
        const to3 = `${currentYear}-${currentMonth
          .toString()
          .padStart(2, "0")}`;
      ...
      case "currentYear":
        setDateRange({
          mesDesde: `${currentYear}-01`,
          mesHasta: `${currentYear}-12`,
        });
        break;
      default:
    ...
    <div className="page-container">
      ...
      <div className="filter-panel">
        ...
        <div className="filter-form">
          ...
          <button type="button" className="theme-button" onClick={clearFilters}>
            🗑️ Limpiar Filtros
          </button>
        </div>
        ...
        {(dateRange.mesDesde || dateRange.mesHasta) && (
          <div className="filter-info">
            <p className="filter-active">
              📊 Filtro activo:
              {dateRange.mesDesde &&
                ` Desde ${formatMonth(dateRange.mesDesde)}`}
              {dateRange.mesHasta &&
                ` Hasta ${formatMonth(dateRange.mesHasta)}`}
            </p>
          </div>
        )}
      </div>
            ...
            <BarChart
              ...
              <Tooltip
                formatter={(value, name) => {
                  if (name === "ganancia")
                    return [`S/. ${value.toFixed(2)}`, "Ganancia"];
                  if (name === "venta")
                    return [`S/. ${value.toFixed(2)}`, "Ventas"];
                  if (name === "costo")
                    return [`S/. ${value.toFixed(2)}`, "Costos"];
                  return [value, name];
                }}
      ...
      {selectedCategory !== "all" && productosDeCategoria.length > 0 && (
        <div className="chart-container">
          <h3 className="chart-title">
            🏷️ Productos de la categoría:{" "}
            <span style={{ color: "#ff6ba6" }}>{selectedCategory}</span>
          </h3>
              ...
              <div key={index} className="product-card">
                ...
                <div className="product-stats">
                  ...
                  <div className="stat-item">
                    ...
                    <span className="stat-value margin">
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredData.ventasPorCategoria.map((categoria, index) => {
                ...
                const getCategoryIcon = (categoryName) => {
                  ...
                  if (name.includes("cupcake") || name.includes("mini"))
                    return "🧁";
                    ...
                    <td style={{ textAlign: "center" }}>
                      <div
                        ...
                        <div
                          style={{
                            width: `${
                              (categoria.monto / getTotalVentas()) * 100
                            }%`,
                            maxWidth: "60px",
                            height: "8px",
                            backgroundColor: COLORS[index % COLORS.length],
                        ...
                        <span style={{ fontWeight: "bold" }}>
                          {((categoria.monto / getTotalVentas()) * 100).toFixed(
                            1
                          )}
                          %
                        </span>
            ...
            <tfoot>
              <tr style={{ backgroundColor: "#f8f9fa", fontWeight: "bold" }}>
                <td>📊 TOTAL GENERAL</td>
                <td style={{ textAlign: "center" }}>
                  {filteredData.ventasPorCategoria.reduce(
