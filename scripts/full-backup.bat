...
echo [PROGRESS] 📂 Copiando backend (excluyendo target)...
xcopy "%PROJECT_DIR%\backend" "%BACKUP_DIR%\source\backend" /E /I /H /Y /Q
if exist "%BACKUP_DIR%\source\backend\target" rmdir /s /q "%BACKUP_DIR%\source\backend\target"
if exist "%BACKUP_DIR%\source\backend\logs" rmdir /s /q "%BACKUP_DIR%\source\backend\logs"

echo [PROGRESS] 📂 Copiando frontend (excluyendo node_modules)...
xcopy "%PROJECT_DIR%\frontend" "%BACKUP_DIR%\source\frontend" /E /I /H /Y /Q
if exist "%BACKUP_DIR%\source\frontend\node_modules" rmdir /s /q "%BACKUP_DIR%\source\frontend\node_modules"
if exist "%BACKUP_DIR%\source\frontend\build" rmdir /s /q "%BACKUP_DIR%\source\frontend\build"
if exist "%BACKUP_DIR%\source\frontend\dist" rmdir /s /q "%BACKUP_DIR%\source\frontend\dist"
...
echo.
echo [STEP 7/7] 📦 Creando archivo ZIP comprimido...
...
if exist "%BACKUP_ROOT%\ComoEnCasa_Full_Backup_%DATE_TIME%.zip" (
    ...
    echo [CLEANUP] 🧹 Eliminando carpeta temporal...
    rmdir /s /q "%BACKUP_DIR%"
    echo [SUCCESS] ✅ Carpeta temporal eliminada - solo queda el ZIP
) else (
...
echo.
echo [EXCLUDED] 🚫 Carpetas excluidas para ahorrar espacio:
echo   • node_modules/ (frontend)
echo   • target/ (backend)
echo   • build/, dist/ (compilados)
echo   • logs/ (logs)
echo.
echo [SIZE] 📊 El backup está optimizado sin dependencias pesadas
